...
cd GameStore.Api
dotnet run
This will automatically restore dependencies, apply database migrations, and start the API. The API will start at: http://localhost:5159
...
Bootstrap 5
Development Tips
Hot Reload: Both the API and React app support hot reload during development
Database: The SQLite database file (GameStore.db) will be created in the API project directory
API Testing: Use the games.http file for quick API testing in VS Code with the REST Client extension
Troubleshooting
Port Conflicts
...
Check the API URL configuration in the React app's client files
CORS is properly configured in the API
